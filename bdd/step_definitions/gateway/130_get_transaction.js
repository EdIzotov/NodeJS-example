...
module.exports = createLibrary()
    ...
    .when(
        ...
        function(amount) {
            ...
            return apiGatewayHelper
                ...
                .then(res => {
                    expect(res.status).to.be.equal(200);
                    expect(res.message).to.have.ownProperty("data");
                    expect(
                        res.message.data.transaction_details
    ...
    .when(
        "we perform the get transaction details request specifying the same transaction ID we used to make the transfer",
        function() {
            return apiGatewayHelper
                .getTransaction(
                    variables.owner.body.iovMasterChainId,
    ...
    .then(
        ...
        function(balanceChange, amountChange) {
            expect(
                variables.result.message.data.transaction_details
            ).to.have.ownProperty(balanceChange);
            expect(
                variables.result.message.data.transaction_details[balanceChange]
            ).to.equal(amountChange);
        }
    )
    .when(
        "we perform the get transaction details request on this account by specifying a random transaction ID",
        function() {
            const wrongTrxId = "wrong_transaction_id";
            return apiGatewayHelper
                .getTransaction(
                    variables.owner.body.iovMasterChainId,
                    keyRef,
