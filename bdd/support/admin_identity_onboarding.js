...
class IovOnboarding {
    ...
    pushAllNodeIdentities() {
        const request = world.getAdminPortalConn();
        const portalAuthHeader = world.getPortalAuthHeader();
        const req = world.portalConfig.api.pushNodeIdentities;

        return request[req.method](
            req.path.replace("{regionId}", this.region.id)
        )
            .set(portalAuthHeader, this.portalSessionToken)
            .then(res => {
                res.body.body.forEach(function (el) {
                    storage.add("IOV_PUBLIC_KEYS", el);
                });
                return res.body;
            });
    }
    ...
    envPrepare(done) {
        return this.authPortal()
            ...
            .then(saps => {
                this.saps = saps;
                return this.pushAllNodeIdentities();
            })
            .then(_ => {
    ...
    createAdminIdentity() {
        ...
        return request[req.method](req.path)
            ...
            .then(res => {
                if (res.error) {
                    logger.error("res.text: ", res.text);
                    throw res.error;
                }
                //console.log("text is ", res.text);
                return common.parseJson(res, "create admin identity").body;
            });
    }
