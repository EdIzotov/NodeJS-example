    ...
    async.waterfall(
        ...
        err => {
            if (err) {
                ...
                process.env["200_trxAmount"] = 10; // amount for all transactions in group

                accounts.forEach((account, n) => {
                    process.env["200_account" + (n + 1)] = JSON.stringify(
                        account
                    );
                    process.env["200_secretKey" + (n + 1)] =
                        secretKeys[n].secretKey;
                    process.env["200_amount" + (n + 1)] = balanceAccount;
                    process.env["200_keyId" + (n + 1)] = secretKeys[n].keyId;
                });
