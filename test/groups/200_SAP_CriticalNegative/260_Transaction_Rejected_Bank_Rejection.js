...
function requests(resolve) {
    ...
    let account3 = JSON.parse(process.env["200_account3"]);

    async.waterfall(
        [
            done => {
                logger.info("Request: CBS delete account");

                cbsApi["Account"].delete(account3, function(err, res) {
                    if (err) {
                        done(err);
                    } else {
                ...
                logger.info("Request: SAP transaction from deleted account.");

                let data = SAP_DATA[TYPES.TRX]({
                    yourIdentityIOVAddress: account3.identity.iovMasterChainId,
                    privKey: privKey,
                    yourWalletAccountNumber: account3.iovSideChainId,
