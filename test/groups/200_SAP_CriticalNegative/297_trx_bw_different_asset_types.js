...
function requests(resolve) {
    let keyId = process.env["200_keyId7"];
    let account = JSON.parse(process.env["200_account7"]);
    ...
    async.waterfall(
        [
                ...
                const allowedAssetTypes = [2];
                const identity = account.identity;

                logger.info(
                    "Request: create accounts with different assetTypes"
                );
                ...
                logger.info(
                    `Request: transaction bw accounts with different asset types`
                );
