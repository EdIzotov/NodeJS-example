...
function requests(resolve) {
    let account = JSON.parse(process.env["600_non_ahg_account10"]);
    let holdAmount = 1;
    ...
    async.waterfall(
        [
                ...
                logger.info(
                    `Request: hold transaction, must be unsuccessful, ${JSON.stringify(
                        data
                    )}`
                );

                cbsApi["oldWorld"].holdTrx(account, data, (err, res) => {
                    if (res) {
                        logger.error(res);
                    }
