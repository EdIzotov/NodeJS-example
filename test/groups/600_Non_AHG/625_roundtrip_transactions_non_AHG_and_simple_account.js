...
function requests(resolve) {
    let amount1 = parseFloat(process.env["600_amount6"]);
    let amount2 = parseFloat(process.env["600_amount7"]);
    let amount_non_ahg1 = parseFloat(process.env["600_amount_non_ahg6"]);
    let amount_non_ahg2 = parseFloat(process.env["600_amount_non_ahg7"]);
    let keyId1 = process.env["600_keyId6"];
    let keyId2 = process.env["600_keyId7"];
    let trxAmount = parseFloat(process.env["600_trxAmount"]);
    let non_ahg_account1 = JSON.parse(process.env["600_non_ahg_account6"]);
    let non_ahg_account2 = JSON.parse(process.env["600_non_ahg_account7"]);
    let simple_account1 = JSON.parse(process.env["600_account6"]);
    let simple_account2 = JSON.parse(process.env["600_account7"]);

    async.waterfall(
        [
            done => {
                logger.info(
                    "Request: perform transaction, from simple account to non-ahg account"
                );

                let data = SAP_DATA[TYPES.TRX]({
                    yourIdentityIOVAddress:
                        simple_account1.identity.iovMasterChainId,
                    privKey: privKey,
                    yourWalletAccountNumber: simple_account1.iovSideChainId,
                    counterpartyAccountNumber: non_ahg_account2.iovSideChainId,
                    amount: trxAmount,
                ...
                logger.info("Request: get balance account1");
                ...
                new sapTransport(url).send(JSON.stringify(data), (err, res) => {
                    if (err) {
                        ...
                        expect(res.body.current).to.equal(amount1 - trxAmount);
                ...
                logger.info("Request: get balance account2");

                let data = SAP_DATA[TYPES.BALANCE]({
                    yourIdentityIOVAddress:
                        non_ahg_account2.identity.iovMasterChainId,
                    privKey: privKey,
                    yourWalletAccountNumber: non_ahg_account2.iovSideChainId,
                    keyId: keyId2
                });
                ...
                new sapTransport(url).send(JSON.stringify(data), (err, res) => {
                    if (err) {
                        ...
                        expect(res).to.have.ownProperty("body");
                        expect(res.body).to.have.ownProperty("current");
                        expect(res.body.current).to.equal(amount2 + trxAmount);
                ...
                logger.info(
                    "Request: perform transaction, from non-ahg account to simple account"
                );

                let data = SAP_DATA[TYPES.TRX]({
                    yourIdentityIOVAddress:
                        non_ahg_account1.identity.iovMasterChainId,
                    privKey: privKey,
                    yourWalletAccountNumber: non_ahg_account1.iovSideChainId,
                    counterpartyAccountNumber: simple_account2.iovSideChainId,
                    amount: trxAmount,
                    valuationTime: Date.now(),
                    keyId: keyId1
                });
                ...
                let data = SAP_DATA[TYPES.BALANCE]({
                    yourIdentityIOVAddress:
                        simple_account2.identity.iovMasterChainId,
                    privKey: privKey,
                    yourWalletAccountNumber: simple_account2.iovSideChainId,
                    keyId: keyId2
                });
