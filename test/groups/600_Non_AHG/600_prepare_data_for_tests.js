...
function requests(resolve) {
    ...
    async.waterfall(
        [
            ...
            (bank, done) => {
                let createIdentities = 11;

                logger.info(`Requests: create ${createIdentities} identities`);
                ...
                logger.info("Requests: create account for identities");
                ...
                logger.info("Requests: create non-AHG account for identities");
                        ...
                        cbsApi["Account"].createNew(
                            ...
                            (err, res) => {
                                if (err) {
                                    ...
                                    expect(res.body.identity.id).to.be.above(0);
                                    expect(res.body.isOnChainOnly).equal(true);

                                    non_ahg_accounts.push(res.body);
                                    next();
                                }
                            }
                        );
                ...
                logger.info("Requests: get QR for identities");
                ...
                logger.info("Requests: add pubkey for identities");
        ...
        err => {
            if (err) {
                ...
                process.env["600_trxAmount"] = 1; // amount for all transactions in group

                accounts.forEach((account, n) => {
                    process.env["600_account" + (n + 1)] = JSON.stringify(
                        account
                    );
                    process.env[
                        "600_non_ahg_account" + (n + 1)
                    ] = JSON.stringify(non_ahg_accounts[n]);
                    process.env["600_secretKey" + (n + 1)] =
                        secretKeys[n].secretKey;
                    process.env["600_amount" + (n + 1)] = balanceAccount;
                    process.env[
                        "600_amount_non_ahg" + (n + 1)
                    ] = balanceAccount;

                    process.env["600_keyId" + (n + 1)] = secretKeys[n].keyId;
                });
