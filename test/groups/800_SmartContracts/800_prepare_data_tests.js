...
function requests(resolve) {
    ...
    async.waterfall(
        [
                ...
                logger.info("Request: create bank");
            ...
            (bank, done) => {
                ...
                logger.info("Request: create account for identities");
                ...
                logger.info("Request: add pubkey for identities");
        ...
        err => {
            if (err) {
                ...
                process.env["800_trxAmount"] = 10; // amount for all transactions in group

                accounts.forEach((account, n) => {
                    process.env["800_account" + (n + 1)] = JSON.stringify(
                        account
                    );
                    process.env["800_secretKey" + (n + 1)] =
                        secretKeys[n].secretKey;
                    process.env["800_amount" + (n + 1)] = balanceAccount;
                    process.env["800_keyId" + (n + 1)] = secretKeys[n].keyId;
                });
