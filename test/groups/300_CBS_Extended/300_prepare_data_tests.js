    ...
    async.waterfall(
        [
            ...
            (bank, done) => {
                ...
                logger.info("Request: get QR(secret) for identities");
        ...
        err => {
            if (err) {
                ...
                process.env["300_trxAmount"] = 10; // amount for all transactions in group

                process.env["300_bank"] = JSON.stringify(bank);

                accounts.forEach((account, n) => {
                    process.env["300_account" + (n + 1)] = JSON.stringify(
                        account
                    );
                    process.env["300_secretKey" + (n + 1)] =
                        secretKeys[n].secretKey;
                    process.env["300_amount" + (n + 1)] = balanceAccount;
                    process.env["300_keyId" + (n + 1)] = secretKeys[n].keyId;
                });
