    ...
    async.waterfall(
        ...
        err => {
            if (err) {
                ...
                process.env["550_trxAmount"] = 1; // amount for all transactions in group

                accounts.forEach((account, n) => {
                    process.env["550_account" + (n + 1)] = JSON.stringify(
                        account
                    );
                    process.env["550_secretKey" + (n + 1)] =
                        secretKeys[n].secretKey;
                    process.env["550_amount" + (n + 1)] = balanceAccount;
                    process.env["550_keyId" + (n + 1)] = secretKeys[n].keyId;
                });
