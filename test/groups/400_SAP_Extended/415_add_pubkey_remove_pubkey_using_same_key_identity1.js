...
function requests(resolve) {
    let keyId = process.env["400_keyId4"];
    let newKeyId;
    let account = JSON.parse(process.env["400_account4"]);
    let amount = parseFloat(process.env["400_amount4"]);
    ...
    async.waterfall(
        [
                ...
                logger.info("Request: CBS create new QR for identity1");
            ...
            (secretKey, done) => {
                ...
                logger.info("Request: SAP addpubkey for identity1");
                ...
                logger.info(
                    "Request: SAP balance. It is necessary to check a new public key. The balance must be " +
                        amount +
                        " EUR"
                );
                ...
                // pubkey using same pubkey

                logger.info(
                    "Request: SAP delete public key using the same key"
                );
