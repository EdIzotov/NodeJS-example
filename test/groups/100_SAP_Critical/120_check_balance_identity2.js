...
function requests(resolve) {
    async.waterfall(
        [
                ...
                let account = JSON.parse(process.env["100_account2"]);
                let secretKey = process.env["100_secretKey2"];
                let keyId = process.env["100_keyId2"];
                let amount = parseFloat(process.env["100_amount2"]);
                ...
                logger.info(
                    "Request: SAP balance for verify the operability of the RSA keys identity1"
                );
