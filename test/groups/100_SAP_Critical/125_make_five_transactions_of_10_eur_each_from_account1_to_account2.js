...
function requests(resolve) {
    var trxAmount = parseFloat(process.env["100_trxAmount"]);
    ...
    function transaction(done) {
        ...
        let secretKey1 = process.env["100_secretKey1"];
        let keyId1 = process.env["100_keyId1"];
        let amount1 = parseFloat(process.env["100_amount1"]);

        let account2 = JSON.parse(process.env["100_account2"]);
        let secretKey2 = process.env["100_secretKey2"];
        let keyId2 = process.env["100_keyId2"];
        let amount2 = parseFloat(process.env["100_amount2"]);

        logger.info(
            "Requests: SAP trx. Perform five transactions " +
                trxAmount +
                " EUR each"
        );
        ...
        new sapTransport(url).send(JSON.stringify(data), (err, res) => {
            if (err) {
                ...
                expect(res.body.currentBalance).to.equal(amount1 - trxAmount);

                process.env["100_trx_id"] = res.id;
                process.env["100_amount1"] = amount1 - trxAmount;
                process.env["100_amount2"] = amount2 + trxAmount;
