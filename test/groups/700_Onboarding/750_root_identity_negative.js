...
if (fs.existsSync(__dirname + "/../../../iov_env.conf")) {
    ...
    // Admin Portal & redbox in same host. change port
...
describe(testHost, function() {
    describe(testOperation, function() {
        ...
        it("Request: get signature to create Group identity", function(resolve) {
            requestId = uuidV1();
            const data = Buffer.concat([
                Buffer.from(rootIdentity.iovAddress),
                Buffer.from(rootIdentity.keyId),
                Buffer.from(requestId),
                Buffer.from("identity"),
                Buffer.from(groupIdentityHash),
                Buffer.from([groupIdentityAssetType]),
                Buffer.from([groupIdentityAssetSubType]),
                Buffer.from(groupIdentityDescription)
            ]).toString("base64");
        ...
        it("Request: create Group identity (operations should not be successful)", function(resolve) {
            let data = {
                auth: {
                    identity: rootIdentity.iovAddress,
                    signature: signature,
                ...
                requestId: requestId,
                body: {
                    hash: groupIdentityHash,
                    assetType: groupIdentityAssetType,
                    assetSubType: groupIdentityAssetSubType,
                    description: groupIdentityDescription
                }
            };
