...
describe('Checking RollBack - IOV-695', function () {
  before(function (done) {
    async.waterfall([
      ...
      next => {
        ...
        cbsApi['Identity'].createNewPrivateIdentity(identity, (err, res) => {
          if (err) {
            done(err);
          } else {
            expect(res).to.be.an('object');
      ...
      next => {
        ...
        cbsApi['Identity'].readIOV({ id: account1.identity.id }, function (err, res) {
          if (err) {
            done(err);
          } else {
            expect(res).to.be.an('object');
      ...
      next => {
        ...
        cbsApi['Identity'].readIOV({ id: account2.identity.id }, function (err, res) {
          if (err) {
            done(err);
          } else {
            expect(res).to.be.an('object');
      ...
      next => {
        ...
        cbsApi['Identity'].createAccountsQR(identity1, (err, res) => {
          if (err) {
            logger.error(err);
            done(err);
          } else {
      ...
      next => {
        ...
        cbsApi['Identity'].createAccountsQR(identity2, (err, res) => {
          if (err) {
            logger.error(err);
            done(err);
          } else {
      ...
      next => {
        ...
        new sapTransport(url).send(JSON.stringify(data2), (err, res) => {
          if (err) {
            done(err);
          } else {
            if (res && res.error) {
  ...
  it('Check correct transaction from account1 to account2', function (done) {
    ...
    logger.info('Transaction of amount - ' + trxAmount + ' from account1 to account2 without description');
    new sapTransport(url).send(JSON.stringify(data3), (err, res) => {
      if (err) {
        ...
        logger.info('Check balance of account1 after correct transaction without description');
        ...
        new sapTransport(url).send(JSON.stringify(data4), (err, res) => {
          if (err) {
            done(err);
          } else {
            if (res && res.error) {
            ...
            logger.info('Check balance of account2 after correct transaction without description');
            ...
            new sapTransport(url).send(JSON.stringify(data5), (err, res) => {
              if (err) {
                done(err);
              } else {
                if (res && res.error) {
                ...
                expect(res).to.have.ownProperty('body');
                expect(res.body).to.have.ownProperty('current');
                expect(res.body.current).to.equal(ammount2);
                done();
              }
            });
          }
  ...
  it('Check correct transaction from account1 to account2 with common description', function (done) {
    ...
    logger.info('Transaction of amount - ' + trxAmount + ' from account1 to account2 with common description');
    new sapTransport(url).send(JSON.stringify(data), (err, res) => {
      if (err) {
        done(err);
      } else {
        ...
        ammount2 = ammount2 + amountPerTransaction;
        expect(res.body.currentBalance).to.equal(ammount1);
        logger.info('Check balance of account1 after correct transaction with common description');
        ...
        new sapTransport(url).send(JSON.stringify(data6), (err, res) => {
          if (err) {
            done(err);
          } else {
            if (res && res.error) {
            ...
            expect(res.body).to.have.ownProperty('current');
            expect(res.body.current).to.equal(ammount1);
            logger.info('Check balance of account2 after correct transaction with common description');
            ...
            new sapTransport(url).send(JSON.stringify(data7), (err, res) => {
              if (err) {
                done(err);
              } else {
                if (res && res.error) {
  ...
  it('Check correct transaction from account1 to account2 with special description', function (done) {
    ...
    let data = SAP_DATA[TYPES.TRX]({
      ...
      amount: trxAmount,
      valuationTime: Date.now(),
      description: 'test description: slave negative confirmation',
      keyId: 1
    });
    logger.info('Transaction of amount - ' + trxAmount + ' from account1 to account2 with special description');
    new sapTransport(url).send(JSON.stringify(data), (err, res) => {
      if (err) {
        done(err);
      } else {
        ...
        logger.info('Check balance of account1 after dropped transaction with special description');
        ...
        new sapTransport(url).send(JSON.stringify(data8), (err, res) => {
          if (err) {
            done(err);
          } else {
            if (res && res.error) {
            ...
            expect(res.body).to.have.ownProperty('current');
            expect(res.body.current).to.equal(ammount1);
            logger.info('Check balance of account2 after dropped transaction with special description');
            ...
            new sapTransport(url).send(JSON.stringify(data9), (err, res) => {
              if (err) {
                done(err);
              } else {
                if (res && res.error) {
                ...
                expect(res.body).to.have.ownProperty('current');
                expect(res.body.current).to.equal(ammount2);
                logger.info('Check balance of account1 after dropped transaction with special description - all nodes');
                let data10 = SAP_DATA[TYPES.BALANCEREPORT]({
                  yourIdentityIOVAddress: account1.identity.iovMasterChainId,
                  privKey: privKey,
                  yourWalletAccountNumber: account1.iovSideChainId,
                  keyId: 1
                });
                new sapTransport(url).send(JSON.stringify(data10), (err, res) => {
                  if (err) {
                    done(err);
                  } else {
                    expect(res).to.be.an('object');
                    expect(res).to.have.ownProperty('id');
                    expect(res.id).to.equal(data10.id);
                    expect(res).to.have.ownProperty('type');
                    expect(res.type).to.equal(data10.type);
                    expect(res).to.not.have.ownProperty('error');
                    expect(res).to.have.ownProperty('body');
                    expect(res.body).to.have.ownProperty('entries');
                    expect(res.body.entries).to.be.an('array');
                    for (let i = 0; i < res.body.entries.length; i++) {
                      expect(res.body.entries[i].current).to.be.equal(ammount1);
                    }
                    logger.info('Check balance of account2 after dropped transaction with special description - all nodes');
                    let data11 = SAP_DATA[TYPES.BALANCEREPORT]({
                      yourIdentityIOVAddress: account2.identity.iovMasterChainId,
                      privKey: privKey,
                      yourWalletAccountNumber: account2.iovSideChainId,
                      keyId: 1
                    });
                    new sapTransport(url).send(JSON.stringify(data11), (err, res) => {
                      if (err) {
                        done(err);
                      } else {
                        expect(res).to.be.an('object');
                        expect(res).to.have.ownProperty('id');
                        expect(res.id).to.equal(data11.id);
                        expect(res).to.have.ownProperty('type');
                        expect(res.type).to.equal(data11.type);
                        expect(res).to.not.have.ownProperty('error');
                        expect(res).to.have.ownProperty('body');
                        expect(res.body).to.have.ownProperty('entries');
                        expect(res.body.entries).to.be.an('array');
                        for (let i = 0; i < res.body.entries.length; i++) {
                          expect(res.body.entries[i].current).to.be.equal(ammount2);
                        }
                        done();
                      }
                    });
