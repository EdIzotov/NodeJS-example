    ...
    function doTest() {
        async.waterfall(
            [
                    ...
                    logger.info(
                        `Transaction from region1 to region2 via SAP ( ${sapUrlRegion1} )`
                    );
                ...
                (trxId, done) => {
                    logger.info(
                        `Check trx status of first from region1 via region1 SAP (${sapUrlRegion1}) `
                    );

                    let accountsRegion1 = accounts[0];
                    let account = accountsRegion1[0];
                ...
                (trxId, done) => {
                    ...
                    logger.info(
                        `Check balance of account from region1 via region1 SAP (${sapUrlRegion1}) `
                    );
                    ...
                    logger.info(`Check balance via CBS ${cbsUrlRegion1}`);
                    ...
                    cbsApi["Account"].read({ id: account.id }, function(
                        err,
                        res
                    ) {
                        if (err) {
                    ...
                    logger.info(`Check balance via CBS ${cbsUrlRegion2}`);

                    let accountsRegion2 = accounts[1];
                    let account = accountsRegion2[0];

                    cbsApi.setCbsUrl(cbsUrlRegion2);
