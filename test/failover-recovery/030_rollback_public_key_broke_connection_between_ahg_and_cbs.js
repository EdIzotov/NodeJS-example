...
describe(testName, function() {
    ...
    it("Drop connection between ahg and cbs", function(resolve) {
        let cmd = "sudo " + dropIp(failoverConfig.cbs[0]).join(" && ");
    ...
    it("Add pubkey. Operations should not be successful.", function(resolve) {
        let account = JSON.parse(process.env["account3"]);
        let secretKey = process.env["secretKey3"];
        let amount = parseFloat(process.env["balanceAccount3"]);
        let keyId = "1";
    ...
    it("Allow connection between shard and sharder", function(resolve) {
        let cmd = "sudo " + acceptIp(failoverConfig.cbs[0]).join(" && ");
